...
class YardController {
    static getYard = async (req, res, next) => {
        try {
            const results = await YardModel.getRoles({ ...req.query })
    ...
    static createYard = async (req, res, next) => {
        try {
            ...
            const result = await YardModel.create({
                userId, address, name, open_time, image, lag_lat
            })
            if (!result) throw new Error("Can't create yard");
            res.status(200).json({
                message: 'Create yard success!',
                data: result
            })
        }
        catch (err) {
